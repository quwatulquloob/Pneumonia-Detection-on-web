...
def symptoms_form():
    ...
    if request.method == 'POST':
        ...
        symptoms = {
            ...
            'Nausea and vomiting': request.form.get('Nausea and vomiting', ''),
            'Confusion or changes in mental status': request.form.get('Confusion or changes in mental status', ''),
            'Loss of appetite': request.form.get('Loss of appetite', ''),
            'Headache': request.form.get('Headache', ''),
            'Muscle pain': request.form.get('Muscle pain', '')
        }
...
def upload():
    if request.method == 'POST':
        ...
        str3 = 'Upload another image'

        # Apply conditions
        if status == "Pneumonia" and preds == 1:
            return str1
        elif status in ["Uncertain", "Mostly Pneumonia"] and preds == 0:
            return str2
        elif status in ["Uncertain", "Mostly Pneumonia"] and preds == 1:
            return str1
            ...
            return str3  # Default to "Upload another image" if conditions are not met
